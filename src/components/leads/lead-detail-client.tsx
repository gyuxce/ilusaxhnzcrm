...
export function LeadDetailClient({
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-6 glass-card rounded-2xl border border-white/5 relative overflow-hidden">
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-purple-500/10 text-purple-400 flex items-center justify-center font-bold text-xl glow-purple border border-purple-500/10">
            {lead.full_name[0]?.toUpperCase()}
          </div>
          <div>
            <h1 className="text-2xl font-black text-white">{lead.full_name}</h1>
            <p className="text-sm text-white/40 mt-0.5">{lead.whatsapp_number} | PIC: {pics.find(p => p.id === lead.assigned_cro_id)?.name || 'Belum di-assign'}</p>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/20 transition-all cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold text-white/70 bg-white/5 border border-white/10 hover:bg-white/10 transition-all cursor-pointer"
          >
            <Edit size={14} />
            Edit Profile
          </button>
      ...
      {/* Tabs */}
      <div className="flex gap-1 p-1 rounded-xl bg-slate-900 border border-white/5 w-fit">
        {[
          ...
          return (
            <button
              ...
              className={cn(
                ...
                active ? "bg-purple-500 text-white shadow-lg glow-purple" : "text-white/50 hover:text-white/80"
              )}
            >
              <Icon size={13} />
              {tab.label}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-card rounded-2xl p-6 border border-white/5 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={item.label} className="p-3 rounded-xl border border-white/5 bg-white/[0.01]">
                    <span className="text-[10px] text-white/30 font-bold uppercase">{item.label}</span>
                    <p className="text-sm font-semibold text-white/80 mt-1">{item.value}</p>
                  </div>
                ))}
              </div>

              <div className="p-4 rounded-xl border border-white/5 bg-white/[0.01]">
                <span className="text-[10px] text-white/30 font-bold uppercase">Catatan / Keterangan</span>
                <p className="text-xs text-white/60 mt-2 leading-relaxed whitespace-pre-wrap">{lead.notes || 'Tidak ada catatan.'}</p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="glass-card rounded-2xl p-6 border border-white/5 space-y-4">
                ...
                <div className="space-y-3">
                  {payments.length === 0 ? (
                    <p className="text-white/30 text-xs text-center py-6">Belum ada catatan pembayaran.</p>
                  ) : (
                    payments.map(p => (
                      <div key={p.id} className="flex items-center justify-between p-4 rounded-xl border border-white/5 bg-white/[0.01]">
                        <div className="space-y-1">
                          <span className="text-xs font-bold text-white uppercase">{p.payment_type}</span>
                          <p className="text-[10px] text-white/40">Tanggal: {p.payment_date} | Metode: {p.payment_method}</p>
                          {p.notes && <p className="text-[11px] text-white/60 italic">Catatan: {p.notes}</p>}
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-extrabold text-emerald-400">Rp {Number(p.amount).toLocaleString('id-ID')}</p>
              ...
              {/* Add Payment Form */}
              <div className="glass-card rounded-2xl p-6 border border-white/5 space-y-4">
                <h3 className="text-white font-bold text-xs uppercase tracking-wider">Catat Pembayaran Baru</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] text-white/40 font-bold uppercase mb-1">Tipe Pembayaran</label>
                    <select
                      value={paymentType}
                  ...
                  <div>
                    <label className="block text-[10px] text-white/40 font-bold uppercase mb-1">Nominal Pembayaran</label>
                    <input
                      type="number"
                      value={paymentAmount}
                ...
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1">Catatan Tambahan</label>
                  <input
                    type="text"
          ...
          {activeTab === 'pemetaan' && (
            <div className="glass-card rounded-2xl p-6 border border-white/5 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Status Pengisian Form</label>
                  <select
                    value={formStatus}
                ...
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Jadwal Pemetaan</label>
                  <input
                    type="datetime-local"
                    value={scheduledAt}
              ...
              <div className="grid grid-cols-1 gap-4 pt-4 border-t border-white/5">
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Status Hasil Pemetaan</label>
                  <select
                    value={resultStatus}
                ...
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Catatan / Link Hasil Pemetaan</label>
                  <textarea
                    placeholder="Hasil pemetaan, rekomendasi jalur program, dll..."
                    value={resultNotes}
                    onChange={e => setResultNotes(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl text-xs text-white outline-none h-24"
                    style={{ background: 'hsl(222,47%,12%)', border: '1px solid hsl(222,47%,20%)' }}
                  />
                </div>
              </div>
          ...
          {activeTab === 'expert' && (
            <div className="glass-card rounded-2xl p-6 border border-white/5 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Nama Expert / Konsultan</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Jadwal Konsultasi</label>
                  <input
                    type="datetime-local"
              ...
              <div className="space-y-4 pt-4 border-t border-white/5">
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Hasil Konsultasi</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Rekomendasi Jalur</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[10px] text-white/40 font-bold uppercase mb-1.5">Langkah Selanjutnya (Next Step)</label>
                  <input
                    type="text"
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card rounded-2xl p-5 border border-white/5 space-y-4">
            ...
            <div className="relative pl-4 space-y-4 before:absolute before:left-1.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-white/10 max-h-[60vh] overflow-y-auto pr-1">
              {activities.length === 0 ? (
                <p className="text-white/30 text-[10px] py-4">Belum ada log aktivitas.</p>
              ) : (
                activities.map(act => (
                  <div key={act.id} className="relative space-y-0.5">
                    ...
                    <p className="text-xs text-white/70">{act.description}</p>
      ...
      {isEditingCore && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
          <div className="relative w-full max-w-lg glass-card rounded-2xl p-6 border border-white/10 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-bold text-white">Edit Data Utama Lead</h3>
            
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-white/40 mb-1">Nama Lengkap</label>
                <input
                  type="text"
                  value={editName}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-white/40 mb-1">Source Campaign</label>
                <input
                  type="text"
                  value={editSource}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-white/40 mb-1">Status Pipeline</label>
                <select
                  value={editStatus}
              ...
              <div>
                <label className="block text-xs text-white/40 mb-1">PIC CRO</label>
                <select
                  value={editPic}
            ...
            {editStatus === 'Not Interested' && (
              <div>
                <label className="block text-xs text-white/40 mb-1">Alasan Lost / Drop-off</label>
                <select
