...
// ReactNode kept for Field children
...
export default function WorkQueuePage() {
  ...
  return (
    <>
      <Header
        title="Kerjaan CRO"
        subtitle="Pilih lead kiri → isi form kanan → Simpan. Header tetap, isi tidak menjulang ke bawah."
      />
      <div className="w-full px-3 py-3 sm:px-4 font-sans">
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2 text-[11px] font-semibold">
          <p className="text-muted-foreground">
            Antrian kiri · formulir kanan ·{' '}
            <span className="text-foreground">{doneCount}/5 terisi</span>
          </p>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[17.5rem_minmax(0,1fr)] gap-0 xl:gap-0 overflow-hidden rounded-2xl border border-border bg-card xl:h-[calc(100vh-9.5rem)]">
          {/* Queue — ink-tinted so it doesn't blend with the form */}
          <aside className="flex flex-col border-b xl:border-b-0 xl:border-r border-border bg-primary/[0.04] min-h-[14rem] xl:min-h-0">
            <div className="shrink-0 space-y-2 border-b border-border/80 p-3">
              ...
              <select
                ...
                className="w-full rounded-lg border border-border bg-card px-2 py-1.5 text-xs font-semibold outline-none"
              >
            ...
            <div className="flex-1 space-y-1 overflow-y-auto p-2">
              {loading ? (
                ...
                <p className="py-8 text-center text-xs text-muted-foreground">Antrian kosong.</p>
              ) : (
                queueItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="shrink-0 text-[9px] font-bold text-accent">{item.reason}</span>
                      </div>
                      ...
                      <span
                        className={cn(
                          ...
                          getStageBadgeClasses(item.lead.current_status)
                        )}
                      >
                        {itemStage ? `T${itemStage.id}` : 'Keluar'} · {item.lead.current_status}
                      </span>
                    </button>
                  )
                })
              ...
              <>
                <div className="shrink-0 space-y-2 border-b border-border px-3 py-2.5 sm:px-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={cn(
                        'hidden sm:inline-block rounded-md px-2 py-0.5 text-[10px]',
                        getStageBadgeClasses(selectedLead.current_status)
                      )}
                    >
                      {stage ? `T${stage.id} ${stage.labelId}` : selectedLead.current_status}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1" aria-label="Progress langkah">
                      {workSteps.map((step) => (
                        <span
                          key={step.n}
                          title={step.title}
                ...
                <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-4">
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                    <Field label={`1. ${workSteps[0].title}`}>
                      <select
                    ...
                    <Field label={`3. ${workSteps[2].title}`} className="sm:col-span-2">
                      <select
                        value={form.solution_given}
                        onChange={(e) => updateForm('solution_given', e.target.value)}
                        className={cn(
                          'field-input',
                          focusStep === 3 && 'border-accent ring-1 ring-accent/30'
                        )}
                      >
                    ...
                    {!isCloseFlow ? (
                      <>
                        <Field label={`4. ${workSteps[3].title}`}>
                          <select
                        ...
                        <Field label={`5. ${workSteps[4].title} (opsional)`}>
                          <input
                            type="date"
                      ...
                      <>
                        ...
                        <Field label={`5. ${workSteps[4].title}`}>
                          <select
                            value={lostReason}
                            onChange={(e) => setLostReason(e.target.value)}
                            className={cn(
                              'field-input',
                              focusStep === 5 && 'border-accent ring-1 ring-accent/30'
                            )}
                          >
                  ...
                  <details
                    ...
                    className="mt-3 rounded-lg border border-dashed border-border px-3 py-2"
                  >
                    <summary className="cursor-pointer list-none text-xs font-semibold text-muted-foreground [&::-webkit-details-marker]:hidden">
                      Opsi lanjutan (opsional) · {showAdvanced ? 'tutup' : 'buka'}
                    </summary>
                    <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                      ...
                      <Field label="Butuh expert?">
                        <label className="flex h-[38px] items-center gap-2 text-xs">
                          <input
                            type="checkbox"
                ...
                <div className="shrink-0 flex items-center justify-between gap-3 border-t border-border bg-secondary/30 px-3 py-2.5 sm:px-4">
                  ...
                  <button
                    ...
                    className={cn(
                      'inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-semibold disabled:cursor-not-allowed disabled:opacity-50',
                      isCloseFlow
                        ? 'bg-primary text-primary-foreground'
                  ...
                  >
                    ...
                    {isCloseFlow ? 'Simpan · tidak lanjut' : 'Simpan'}
                  </button>
                </div>
              </>
            )}
